...
图形版保留上述机器列表、NPU 状态卡片、可用数量配色、并行查询、查询时间和
倒计时逻辑，界面调度由 Tkinter 自动管理：

```bash
python3 npu_dashboard_tkinter.py
```

右上角可随时开关“窗口置顶”，也可点击主题按钮在“自动”、“深色”和“浅色”
之间循环切换。自动模式会跟随系统外观变化，窗口置顶与主题状态都会持久化。
“设置”可修改远端查询周期、
最大分列数、机器列表、远端命令和 SSH 超时。设置保存在系统默认的用户缓存目录：Windows 使用
